...
public partial class CMSModules_Ecommerce_Controls_ProductOptions_AvastoneCustomShoppingCartItemSelector : CMSUserControl
{
    #region "Variables"
    ...
    #region "Custom Properties Trevor Fayas"

    private string mCategorySelectionTypeColumn;
    public string CategorySelectionTypeColumn
    {
        get { return (string.IsNullOrWhiteSpace(mCategorySelectionTypeColumn) ? "CategorySelectionType" : mCategorySelectionTypeColumn); }
        set { mCategorySelectionTypeColumn = value; }
    }

    private string mCategoryDefaultOptionsColumn;
    public string CategoryDefaultOptionsColumn
    {
        get { return (string.IsNullOrWhiteSpace(mCategoryDefaultOptionsColumn) ? "CategoryDefaultOptions" : mCategoryDefaultOptionsColumn); }
        set { mCategoryDefaultOptionsColumn = value; }
    }

    private string mDefaultProductOptionSelectorControl;
    public string DefaultProductOptionSelectorControl
    {
        get { return (string.IsNullOrWhiteSpace(mDefaultProductOptionSelectorControl) ? "~/CMSModules/Ecommerce/Controls/ProductOptions/ProductOptionSelector.ascx" : mDefaultProductOptionSelectorControl); }
        set { mDefaultProductOptionSelectorControl = value; }
    }


    // Custom Trevor Fayas - Used to regenerate from database these values if Product Options call the OnOptionCategoryModified event.
    // Useful for if your product option needs to generate new product options or skus on add to cart.
    private bool RegenerateVariants;
    private bool RegenerateOptionSkus;
    // End Custom
    ...
    public string ProductOptions
    {
        get
        {
            ...
            {
                // Custom Trevor Fayas - Since GetSelectedSKUOptions is not overwritable, if it's an Avastone version of it then use GetSelectedSKUOptionsCustom();
                options.AppendFormat("{0},", (IsSelectOptionAvastoneCustom(selector) ? ((AvastoneProductOptionSelector)selector).GetSelectedSKUOptionsCustom() : selector.GetSelectedSKUOptions()));
                // End Custom
            }
    ...
    private List<ProductVariant> Variants
    {
        get
        {
            // Custom Trevor Fayas - Allow to override the Variant cache to regenerate.
            if (mVariants == null || RegenerateVariants)
            {
                int cacheMinutes = SettingsKeyInfoProvider.GetIntValue(SiteContext.CurrentSiteName + ".CMSCacheMinutes");

                using (var cs = new CachedSection<List<ProductVariant>>(ref mVariants, cacheMinutes, true, null, "mProductVariants", SKUID))
                {
                    if (cs.LoadData || RegenerateVariants)
                    {
                        // Custom Trevor Fayas - Reset value.
    ...
    private List<Tuple<int, int>> SelectedOptionsInCategories
    {
        get
        {
            if (mSelectedOptionsInCategories == null || RegenerateOptionSkus)
            {
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        // Custom - Trevor Fayas Set values)
        RegenerateVariants = false;
        RegenerateOptionSkus = false;
    ...
    private void AddProductToWishlist()
    {
        ...
        // Custom Trevor Fayas - Call overridden methods of custom form elements
        CallOnAddToWishlistBefore();
        // End Custom
    ...
    #region "Custom Tevor Fayas - Form On Cart and Wishlist Callers"
    private List<AvastoneProductOptionSelector> GetCustomFormOptions()
    {
        List<AvastoneProductOptionSelector> FormItems = new List<AvastoneProductOptionSelector>();
        foreach (ProductOptionSelector OptionSelector in OptionSelectors)
        {
            try {
                FormItems.Add((AvastoneProductOptionSelector)OptionSelector);
            }
            catch (Exception ex)
            { }
        }
    ...
    private bool IsSelectOptionAvastoneCustom(ProductOptionSelector OptionSelector)
    {
        try
        {
            var Test = ((AvastoneProductOptionSelector)OptionSelector);
            return true;
        }
        catch (Exception ex)
        {
    ...
    private void CallOnAddToCartBefore()
    {
        foreach (AvastoneProductOptionSelector OptionSelector in GetCustomFormOptions())
        {
            OptionSelector.OnAddToCartBefore();
        }
    }
    private void CallOnAddToCartAfter()
    {
        foreach (AvastoneProductOptionSelector OptionSelector in GetCustomFormOptions())
        {
            OptionSelector.OnAddToCartAfter();
        }
    }
    private void CallOnAddToWishlistBefore()
    {
        foreach (AvastoneProductOptionSelector OptionSelector in GetCustomFormOptions())
        {
            OptionSelector.OnAddToWishListBefore();
        }
    }
        ...
        if (!DataHelper.DataSourceIsEmpty(dsCategories))
        {
            ...
            foreach (DataRow dr in dsCategories.Tables[0].Rows)
            {
                try
                {
                    ...
                    // Custom Trevor Fayas - If the Product option is set to FormControl, loads the CategoryDefaultRecord (where the form control path is)
                    // Load control for selection product options
                    ProductOptionSelector selector = null;

                    if(dr[CategorySelectionTypeColumn].ToString() == OptionCategorySelectionTypeEnum.FormControl.ToStringRepresentation() && dr[CategoryDefaultOptionsColumn] != DBNull.Value)
                    {
                        selector = (ProductOptionSelector)LoadUserControl(dr[CategoryDefaultOptionsColumn].ToString());
                        // If it's an AvastoneProductOptionSelector selector, add the event hook to re-load Cached items.
                        if (IsSelectOptionAvastoneCustom(selector))
                        {
                            ((AvastoneProductOptionSelector)selector).OptionCategoryModified += AvastoneCustomShoppingCartItemSelector_OnOptionCategoryModified;
                        }
                    } else {
                        selector = (ProductOptionSelector)LoadUserControl(DefaultProductOptionSelectorControl);
                        
                    } 

                    // Trevor Fayas - End Customization
    ...
    void AvastoneCustomShoppingCartItemSelector_OnOptionCategoryModified(object sender, EventArgs e)
    {
        // Reload Cached Data.
        RegenerateOptionSkus = true;
    ...
    private void CalculateVariantData()
    {
        ...
        foreach (ProductOptionSelector selector in OptionSelectors)
        {
            ...
            {
                // Custom Trevor Fayas - Since GetSelectedSKUOptions is not overwritable, if it's an Avastone version of it then use GetSelectedSKUOptionsCustom();
                string option = (IsSelectOptionAvastoneCustom(selector) ? ((AvastoneProductOptionSelector)selector).GetSelectedSKUOptionsCustom() : selector.GetSelectedSKUOptions());
                // End Custom
                ...
                // Custom Trevor Fayas - In our case, the Variant Option Sku is generated on the add, so the ProductOPtionsData doesn't have that new one, resulting in an error that the variant doesnt exist
                // So this will allow us to pull from database to get the new data.

                DataSet dsAllOptions = null;
                if (!RegenerateOptionSkus)
                {
                    // Get all options for this category from selector to reduce SQL queries
                    dsAllOptions = selector.ProductOptionsData;
                }
                else
                {
                    if (RegenerateOptionSkus)
                    ...
                    // This i pull directly as sometimes the Category has a new sku is created then.
                    var SkuFinder = SKUInfoProvider.GetSKUs().Where("SKUEnabled = 1 and SKUOptionCategoryID = " + selector.OptionCategoryId);
                    dsAllOptions = SkuFinder.Execute();
                }
